...
W celu lepszej czytelnoœci, odchylenia standardowe czasów wykonania zosta³y dodatkowo przedstawione na osobnym wykresie.
...
ggplot(data_times, aes(x = inst_size, y = time)) +
  geom_errorbar(aes(ymin = time-se, ymax = time+se), width=.1) +
  facet_wrap(~ name, ncol = 2) +
  ggtitle("Odchylenia standardowe czasów dzia³ania algorytmu w milisekundach") +
  xlab("Rozmiar instancji problemu") +
  ylab("Czas dzia³ania algorytmu") +
  theme_bw()
...
Algorytmy symulowanego wy¿arzania i wyszukiwania tabu charakteryzuj¹ siê du¿o mniejsz¹ stabilnoœci¹ czasu wykonania. Jest to prawdopodobnie spowodowane mo¿liwoœci¹ przechodzenia do s¹siadów pogarszaj¹cych wynik, przez co w bardziej losowy sposób przeszukiwany jest obszar rozwi¹zañ i w niektórych przypadkach program szybciej siê zatrzymuje w optimum lokalnym, a w niektórych póŸniej.
