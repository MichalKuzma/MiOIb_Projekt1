...
ggplot(data_results %>% filter(name == "Random"), aes(x = inst_size, y = min)) +
  geom_point() +
  ggtitle("Minimalna odleg³oœæ uzyskanego przez algorytm losowy rozwi¹zania od optimum") +
  xlab("Rozmiar instancji problemu") +
  ylab("Odleg³oœæ od optimum") +
  geom_smooth(method = "lm") +
  theme_bw()
...
Metod¹ regresji liniowej wyznaczono zale¿noœci miêdzy wielkoœci¹ instancji, a wzglêdn¹ odleg³oœci¹ rozwi¹zania od optimum. Poza algorytmem losowym ¿aden nie wykazuje wyraŸnej zale¿noœci pomiêdzy wielkoœci¹ instancji, a odleg³oœci¹ znalezionego rozwi¹zania od optimum. Random wraz ze wzrostem wielkoœci instancji zwraca rozwi¹zania coraz bli¿sze optymalnemu. Jest to zwi¹zane ze znacznie d³u¿czym czasem wykonywania algorytmu (takim, jak dla przeszukiwania lokalnego na danej instancji).
...
Przedstawione powy¿ej wykresy czasu trwania iteracji algorytmów pokazuj¹ wzrost czasu trwania algorytmu wraz ze wzrostem wielkoœci instancji (czego mo¿na siê by³o spodziewaæ). 
...
data_ef <- data %>%
  mutate(ef = (1 + result_dist + time)/ ((result_dist + 1) * (time + 1))) %>%
...
ggplot(data_ef %>% filter(name == "Random"), aes(x = inst_size, y = ef)) +
  geom_point() +
  ggtitle("Œrednia efektywnoœæ algorytmu losowego dla danego rozmiaru instancji problemu") +
  xlab("Rozmiar instancji problemu") +
  ylab("Efektywnoœæ") +
  geom_smooth(method = "lm") +
  theme_bw()
...
pierwsz¹ obserwacj¹ jest wyraŸnie gorsza efektywnoœæ algorytmu przeszukiwania losowego. Algorytmy *Symulowane wy¿arzanie* i *Tabu Search*, a szczególnie ten pierwszy, wykazuj¹ wiêksz¹ efektywnoœæ dla wielu instancji testowych, ni¿ przeszukiwanie lokalne. Algorytm symulowanego wy¿arzania cechuje siê najmniejsz¹ zmiennoœci¹ efektywnoœci w zale¿noœci od instancji problemu (najmniejsze odchylenia od funkcji uzyskanej z regresji liniowej).
...
Wykres dla pierwszej z badanych instancji przedstawia relacjê malej¹c¹ (im bardziej podobne rozwi¹zania, tym mniejsza ró¿nica w ich jakoœci), podczas, gdy drugi nie przejawia wyraŸnej relacji omawianych wielkoœci. Rozwi¹zania znalezione dla drugiej instancji s¹ równie¿ mniej podobne do siebie wzajemnie -- dla instancji problemu *had12* pary rozwi¹zañ maj¹ podobieñstwo siêgaj¹ce nawet 10 (na maksimum 12), natomiast dla *nug14* najwy¿sze podobieñstwo wynios³o 6/14.
...
Wzglêdna odleg³oœæ znalezionego rozwi¹zania od optimum globalnego jest niezale¿na od wielkoœci instancji, a dla algorytmu *Random Search* jest ona nawet tym mniejsza, im wiêksza jest instancja problemu. Jest to jednak w du¿ej mierze znacznym wyd³u¿eniem czasu dzia³ania RS dla wiêkszych instancji.

Czas wykonywania algorytmów roœnie wraz ze wzrostem wielkoœci instancji. Jest to spodziewany wniosek, jednak nale¿y zauwa¿yæ, ¿e od tej regu³y s¹ wyj¹tki i d³ugoœæ permutacji nie jest jedynym kryterium determinuj¹cym czas wykonywania algorytmu. Najbardziej stabilny czas wykonywania wykazuje *Steepest Local Search*, co jest zrozumia³e bior¹c pod uwagê, ¿e w ka¿dym kroku przeszukuje ca³e dostêpne s¹siedztwo (które ma równ¹ licznoœæ w ka¿dym punkcie przestrzeni rozwi¹zañ).
